...
Runs unattended, with no chat attached, so deliver
...
2. **Compare section by section** to the current doc. An "update" = any real, sourced change (funding, pricing, product, integration, leadership, customers, press…).
3. **Collect changes — don't edit yet.** The unattended run proposes first, and only writes after approval. Never fabricate; skip unchanged sections; note any missing
...
need no approval — the summary message is the whole run.nks.
