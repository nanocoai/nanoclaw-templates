...
function renderHtml(review, project, baseline, document, proposedDocument) {
  const changeCards = review.changes.length === 0
    ...
    : review.changes.map((change) => `
      <article class="change"><strong>${escapeHtml(change.role)}</strong>
        <p><del>${escapeHtml(change.oldText)}</del></p>
        <p><ins>${escapeHtml(change.newText)}</ins></p>
      </article>`).join('');
...
<title>Evidence Domino — ${escapeHtml(review.reviewId)}</title>
<style>
:root{color-scheme:light;--ink:#161616;--muted:#66645f;--line:#ddd9d0;--paper:#fff;--wash:#f6f4ef;--amber:#9a5b00;--amber-bg:#fff4dd;--red:#a12618;--red-bg:#fff0ed;--green:#27603a;--focus:#2357d8}*{box-sizing:border-box}html,body{width:100%;max-width:100%;overflow-x:hidden}body{margin:0;background:var(--wash);color:var(--ink);font:16px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;overflow-wrap:anywhere}main{width:100%;min-width:0;max-width:1400px;margin:auto;padding:32px}.mast{min-width:0;max-width:100%;overflow:hidden;background:var(--paper);border:1px solid var(--line);border-radius:18px;padding:30px}.eyebrow{letter-spacing:.08em;text-transform:uppercase;font-size:.78rem;color:var(--muted)}h1{font:700 clamp(2rem,5vw,4.7rem)/1.02 Georgia,serif;max-width:1050px;margin:.35rem 0 1.2rem;overflow-wrap:anywhere;word-break:break-word}.badges{display:flex;min-width:0;flex-wrap:wrap;gap:8px}.badge{width:max-content;max-width:100%;border-radius:999px;padding:6px 10px;font-weight:700;font-size:.8rem;background:var(--amber-bg);color:var(--amber);white-space:normal}.badge.replay{background:#e9efff;color:#244795}.numbers{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:20px 0}.number{min-width:0;max-width:100%;overflow:hidden;background:var(--paper);border:1px solid var(--line);border-radius:14px;padding:18px}.number small{display:block;color:var(--muted)}.number b{font:700 1.65rem/1.3 Georgia,serif}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-top:16px}.panel{min-width:0;max-width:100%;overflow:hidden;background:var(--paper);border:1px solid var(--line);border-radius:14px;padding:20px}.panel h2{font-size:.82rem;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin:0 0 12px}pre{width:100%;min-width:0;max-width:100%;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;font:1rem/1.55 Georgia,serif;margin:0}.evidence{min-width:0;max-width:100%;border-left:4px solid var(--amber);padding-left:14px}.meta{overflow-wrap:anywhere;word-break:break-word;color:var(--muted);font-size:.86rem}.change{border-top:1px solid var(--line);padding:12px 0}.change:first-child{border-top:0}.change p{margin:.45rem 0}del{color:var(--red);background:var(--red-bg)}ins{text-decoration:none;color:var(--green);background:#edf8f0}details{min-width:0;max-width:100%;overflow:hidden;margin-top:16px;background:var(--paper);border:1px solid var(--line);border-radius:14px;padding:16px}summary{cursor:pointer;font-weight:700}summary:focus-visible{outline:3px solid var(--focus);outline-offset:4px}.warning{color:var(--red);font-weight:700}.quiet{color:var(--muted)}@media(max-width:900px){main{padding:14px}.grid,.numbers{grid-template-columns:minmax(0,1fr)}.mast{padding:20px}.badges{display:grid;grid-template-columns:minmax(0,1fr)}h1{font-size:clamp(2rem,11vw,2.35rem)}}
</style>
</head>
<body><main>
<section class="mast"><div class="eyebrow">Evidence Domino</div><h1>${escapeHtml(review.headline)}</h1>
<div class="badges"><span class="badge">${escapeHtml(review.statusLabel)}</span>${replayBadge}</div>
<p class="meta">Status when generated · ${escapeHtml(review.createdAt)} · document v${baseline.version} · ${escapeHtml(review.reviewId)}</p></section>
<section class="numbers" aria-label="Calculation summary">
<div class="number"><small>Unit price</small><b>${escapeHtml(formatUsd(baseline.unitPriceCents))} → ${escapeHtml(formatUsd(review.interpretation.unitPriceCents))}</b></div>
<div class="number"><small>Remaining before other costs</small><b>${escapeHtml(formatUsd(baseline.calculation.remainingCents))} → ${escapeHtml(formatUsd(review.calculation.remainingCents))}</b></div>
<div class="number"><small>Tracked supplier cost</small><b>${escapeHtml(formatUsd(review.calculation.trackedCostCents))}</b></div>
<div class="number"><small>Conditional shortfall</small><b>${escapeHtml(formatUsd(review.calculation.shortfallCents))}</b></div>
</section>
<section class="grid">
<article class="panel"><h2>Current document</h2><pre>${escapeHtml(document)}</pre></article>
<article class="panel"><h2>Evidence</h2><h3>Previous owner-confirmed passage</h3><div class="evidence"><p>${escapeHtml(baseline.interpretation.supportingPassage)}</p></div><p class="meta"><a href="${escapeHtml(baseline.interpretation.sourceUrl)}" target="_blank" rel="noopener noreferrer">Open previous source</a></p><h3>New Tavily-returned passage</h3><div class="evidence"><p>${escapeHtml(review.interpretation.supportingPassage)}</p></div><p class="meta"><a href="${escapeHtml(review.source.requestedUrl)}" target="_blank" rel="noopener noreferrer">Open new source</a><br>${escapeHtml(review.source.retrievedAt)}<br>${escapeHtml(review.source.captureId)}</p><p class="warning">A public listing is not proof that a supplier contract changed.</p></article>
<article class="panel"><h2>Proposed repair</h2><pre>${escapeHtml(proposedDocument)}</pre></article>
</section>
<details open><summary>Affected tracked sentences</summary>${changeCards}</details>
<details><summary>Calculation and limits</summary><p>${project.quantity.toLocaleString('en-US')} × ${escapeHtml(formatUsd(review.interpretation.unitPriceCents))} = ${escapeHtml(formatUsd(review.calculation.trackedCostCents))}. ${escapeHtml(formatUsd(project.customerQuoteCents))} − ${escapeHtml(formatUsd(review.calculation.trackedCostCents))} = ${escapeHtml(formatUsd(review.calculation.remainingCents))} before other costs.</p><p>Evidence Domino tracks exactly three owner-approved sentences. It does not validate every claim in the proposal. Retrieval time is not publisher modification time. The owner must confirm applicability before adoption.</p></details>
<footer class="panel" style="margin-top:16px"><p><strong>Revision ID:</strong> ${escapeHtml(review.revisionId ?? 'None')}</p>${review.revisionId ? `<p><strong>Exact approval phrase:</strong> ${escapeHtml(`Confirm that this observed price applies and adopt revision ${review.revisionId}.`)}</p>` : ''}<p>This is an immutable snapshot. Ask for current status in chat before acting on an older report.</p></footer>
</main></body></html>`;
}
