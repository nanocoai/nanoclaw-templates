...
confirmation instead of asking cold**, in this shape and no other:
...
**Devices**
1. Robot vacuum
2. Tami4 water dispenser (Kitchen)

Right?
```

One name per line, every area, every device, no prose sentence wrapping them.
Devices are named the way the snapshot names them, with the area in brackets
where it has one.
